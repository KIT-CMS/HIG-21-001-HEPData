...
async def main(n_threads, commands):
    ...
    for i in range(n_threads):
        task = asyncio.ensure_future(execute_command(creation_task_queue, f"create_worker_{i}"))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Script to create postfit shapes for HEPData queue-based")
    parser.add_argument("--n-threads", required=True, type=int, help="Number of threads to be used")
    parser.add_argument("--categories-configurations", required=True, nargs="+", help="Number of threads to be used")
    ...
    for confname in args.categories_configurations:
        ...
        for c in conf["catlist"]:
            ...
            if conf["additional_inputs"] and conf["additional_signals"]:
                additional = " ".join(["--additional-inputs", conf["additional_inputs"].format(CAT=c), "--additional-signals", "'" + conf["additional_signals"] + "'"])
    ...
    main_task = asyncio.ensure_future(main(args.n_threads, commands))
